...
class MechanismAuthCurveClient is Mechanism
  ...
  new create(session: Session, c_sk: CryptoBoxSecretKey, c_pk: CryptoBoxPublicKey, s_pk: CryptoBoxPublicKey) =>
    _session = session
    _c_sk = c_sk
    _c_pk = c_pk
    _s_pk = s_pk
  ...
  fun ref _make_message_writex(): MessageWriteTransform iso^ =>
    ...
    recover
      _CurveMessageWriter(sk, pk, consume nonce_gen, "CurveZMQMESSAGEC")
    end
